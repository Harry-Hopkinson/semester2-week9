...
{
    // visit node data, then first child, and then second child
    if( loc != NULL )
    {
        printf(" Node %d\n",loc->data->value);
...
{   
    // visit first child, then second child, and then node data
    if( loc != NULL )
    {
...
{
    // visit first child, then node data, and then second child
    if( loc != NULL )
    {
